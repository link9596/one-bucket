...
// ==================== 修复1：自定义 InvalidKeyError ====================
// 原来 sanitizeKey 抛普通 Error，路由层未捕获导致返回 500。
// 现在用独立错误类型，由顶层 fetch 统一捕获映射为 400。
class InvalidKeyError extends Error {
  constructor(msg) {
    super(msg || 'Invalid key');
    this.name = 'InvalidKeyError';
  }
}

function sanitizeKey(key) {
  if (typeof key !== 'string') throw new InvalidKeyError('Invalid key');

  // 检测 URL 编码的路径遍历模式（如 %2e%2e%2f）
  if (/%2e|%2E/i.test(key)) {
    throw new InvalidKeyError('Path traversal not allowed');
  }
...
// ==================== 修复2：Endpoint 白名单校验（SSRF 防护） ====================
// 只允许 https 公网域名，禁止 IP、localhost、内网、云元数据域名。
...
// ==================== 实例工厂 ====================
// 修复3：支持传入已加载的 configs，避免重复读取 KV + 重复解密。
async function getBucketInstance(env, bucketId, preloadedConfigs) {
  const configs = preloadedConfigs || await getBucketsConfig(env);
...
async function handleRequest(request, env) {
  ...
  if (path === "/admin/buckets" && method === "GET") {
    ...
    const safe = configs.map(c => ({
      ...
      region: c.region || 'auto',   // 修复4：返回 region
  ...
  if (path === "/admin/buckets" && method === "POST") {
    ...
    const merged = newConfigs.map(newItem => {
      ...
      return {
        ...
        region: newItem.region || oldItem?.region || 'auto',  // 修复4：保留 region
    ...
    for (const c of merged) {
      ...
      // 修复2：校验 endpoint
      if (!validateEndpoint(c.endpoint)) {
        return Response.json({
          code: 400,
          msg: "Endpoint 不合法：必须是 https 公网域名，不允许 IP、localhost 或内网地址"
        }, { status: 400 });
      }
    }
  ...
  if (path === "/usage" && method === "GET") {
    if (!bucketConf) return Response.json({ code: 404, msg: "桶配置不存在" }, { status: 404 });
    const usage = await getBucketUsage(bucketConf.accountId, bucketConf.id, bucketConf.apiToken);
    if (usage === null) {
...
// ==================== 顶层导出：统一捕获 InvalidKeyError ====================
// 修复1：sanitizeKey 抛出的 InvalidKeyError 在这里被捕获并返回 400，
// 而不是像原来那样冒泡成 500。
export default {
  async fetch(request, env) {
    try {
      ...
      if (e instanceof InvalidKeyError) {
        return Response.json(
          { code: 400, msg: '非法路径：' + e.message },
          { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } }
        );
      }
      console.error('Unhandled error:', e);
      return Response.json(
        { code: 500, msg: 'Internal server error: ' + (e && e.message ? e.message : 'unknown') },
